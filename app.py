...
# Ensure database exists to prevent initial load crash
...
class PlaneShopApp:
    def __init__(self, root):
        ...
        # Inventory Treeview Display
        self.tree_frame = tk.Frame(root)
        self.tree_frame.pack(pady=10, fill="x", px=20)
        ...
        # Tabs for Admin Controls vs User Storefront
        self.notebook = ttk.Notebook(root)
        self.notebook.pack(pady=15, fill="both", expand=True, px=20)

        self.setup_admin_tab()
        self.setup_user_tab()

        # Initialize and load current items
        self.update_inventory_display()

        # Save and Save-On-Exit behavior
        self.root.protocol("WM_DELETE_WINDOW", self.on_exit)

    def update_inventory_display(self):
        """Clears and re-populates the inventory viewing table."""
        for item in self.tree.get_children():
            self.tree.delete(item)

        # Repopulate selection dropdowns along with table grid
    ...
    def setup_admin_tab(self):
        """Creates the Admin Interface Tab for adding/removing stock."""
        ...
        tk.Button(btn_frame, text="Add To Stock", bg="#4caf50", fg="white", px=10, command=self.admin_add).pack(side="left", padx=10)
        tk.Button(btn_frame, text="Remove From Stock", bg="#f44336", fg="white", px=10, command=self.admin_remove).pack(side="left", padx=10)

    def setup_user_tab(self):
        """Creates the Shopping storefront interface tab."""
        ...
        tk.Button(user_frame, text="Purchase Items", bg="#2196f3", fg="white", px=20, command=self.user_purchase).grid(row=2, column=0, columnspan=2, pady=15)

    # --- BUTTON UTILITIES & VALIDATION ---
    def get_inputs(self, combo, entry):
        """Validates standard selections and matches your custom try-except errors."""
        item = combo.get()
        if not item:
...
if __name__ == "__main__":
    root = tk.Tk()
    # Explicitly map the first column layout to pull row info visually
